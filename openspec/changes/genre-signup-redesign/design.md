...
**D22. 打开书的默认落点随书的阶段走（用户拍板 2026-09-10，取代 C5/P0-5）。**

**病根**：落点写死 `workbench`（`useWorkbench` 初始 state），于是**刚建的空书**打开后落在写作视图——工作台树是空的、没有任何可写的东西；而建书弹窗的按钮刚改成「创建，去写简介」，承诺与落点不符。另一头：**全部章节已归档**（写完）的书打开也落写作，而那时作者的意图是通读（预览）。

**设计**：落点＝**书阶段的函数**，阶段只看章节（`lib/novelStage.ts` 单源）：
`stageFromChapters(total, archived)` → 无章节＝setting / 全归档＝done / 其余＝writing；
`landingViewFor` → setting→设定、writing→写作、done→预览。
**为什么不用 `current_phase`**：phase 是「最近一次操作」的记账——归档一章即停在 `archive`，那不代表整本写完，用它会把"归档过一章就停手"的书误判成 done 而落预览。
**只应用一次**：首次书树加载后判定，用户/深链已显式选择视图时不覆盖（`setView` 标主动导航；**自动聚焦第一章不算**——它复用 `focusNode`，早期误标导致落点永不生效，已修）；后续刷新/归档不回跳。
**连带**：空书已落设定 → 不再叠「开始设定」引导卡（卡片唯一作用就是把人送到设定）。

**遗留（已由 tasks 6.13/6.14 结清）**：卡片阶段曾按 `current_phase` 派生 → 与落点不一致；2026-09-10 用户拍板「卡片状态应该落在写作」后卡片与落点同源（`stageFromChapters`），卡片判据依赖的 `total_archives` 也归位为「已归档章节数」。

**D23. 01 格＝题材本身（题材目录）（用户拍板 2026-09-10）。**

**病根**：01 格名叫「题材」，装的却是**口味胶囊**（逆袭打脸/烧脑博弈/…）——那是「怎么写」的起点，不是「写什么题材」。作者被问「题材」时看到的是口味，等于**第一问缺失**（题材面板的六格地基没打）。原先的理由是「口味联动能预填 02-05」，但那是把 01 塞满的副产品，不是 01 该有的语义。

**设计**：01 改**题材目录**两级选择——**大类必选**（20 项，用户给定）＋**子类可选**；封闭目录，**以中文名为存储值**（不自造 slug：名字即稳定键，免 id↔名漂移），写入按目录校验（未知大类/跨类子类 → 400）。口味胶囊**移到 02 格**作「常见口味」快捷填充（它填的就是 02-05，名实相符），**不覆盖 01 已选题材**。

**存储**：题材目录落 **`story.yaml`**（大类复用既有 `genre` 键、子类新键 `sub_genre`），对外契约 `GET/PUT /settings/genre` 含 `theme`/`sub_genre`（面板一次取全、一次保存；存储位置对前端透明）。
**为什么是 `story.yaml` 而不是 `novel_genre` 加列**：① 题材目录是与简介同族的**单值书级元数据**（同文件、已有 `genre` 键），关系表其余四张表承载的是**多值**契约（禁项/战场列表）；② **加列改 schema → 触发 C端 启动期指纹留档**（`legacy_archive`：指纹不匹配即把库三件套改名重建）→ **用户本地已有书/设定/AI 配置全部"消失"**（本会话已因指纹重建真实发生过一次，靠手工拷数据救回）。加一列就赔上用户数据，不可接受；③ `story.yaml` 早已是简介真源（`GET/PUT /novels/{id}/story` 直读直写），题材目录并入同族是一致而非破例。
**键存在才写**：PUT 未带这两个键时不改动既有值（老调用方只 PUT 五字段不得清空题材）。

**判据连带**：题材确认＝**已选题材目录大类 或 核心键任一非空**——01 问的就是题材，只选了题材也算题材已定，否则作者被自己答的第一问卡住。

**展示连带**：书卡胶囊/书内标签的展示名改为**题材目录优先**（`大类 · 子类` → 老书核心承诺 → 老书 `story.yaml.genre`/KV → 「待定题材」占位）；写正文注入 `build_genre_section` **加「题材：大类（子类）」一行**（01 是「定了就不跑偏」的类型锁，只注入 02-06 会让模型不知道书是什么题材）。

**未动原型**：01 的两次选择用既有 `.cap`/`.cap-row` 词汇 + 一个组标题 `.cap-label`（原型未建模该标题，属应用侧扩展，不进 parity 截图）。

**D24. 05 改名「本小说斗什么」+ 06 剧情轨道退役并入主线（用户拍板 2026-09-10）。**

**病根**：① 「主线战场」这个词在问「斗什么」这件事上不直白——作者要先解码"战场"是比喻；用户给的问法「本小说斗什么」是同一件事的白话版，且与其余格（主要看什么/绝对禁止/吃苦指数）的疑问句口径一致。② **06「剧情轨道」与「主线规划」是同一个概念**：前者要「起点 → 每卷推进一步 → 终点方向」，后者（story-arc）就是「一句话主线 + 结局 + 分卷推进」——一处两存，作者要写两遍，违反本体纪律（一个概念一个归属）。

**设计**：05 改名；06 整格移除（题材 = 目录 + 四格），**剧情轨道归主线规划**。
**连带（关键）**：主线的 `story_arc.premise` 此前**没有进写作注入**（只有章纲 AI 草稿读它），若只删 06，「整本书怎么走」在写正文时就没信息来源了 → 本次把「全书主线：…」补进写作上下文，退役即无损。
**DB 列 `novel_genre.track` 保留不迁移**（旧值不读不写）——加列/删列会改 schema 指纹触发整库留档（见 D23），不值得为一列冒险。
