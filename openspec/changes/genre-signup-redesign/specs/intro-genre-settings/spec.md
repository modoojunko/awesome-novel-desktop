...
- **02 SHALL 支持「多看点」开关**：`{multi_point}` 由请求体的 `multi_point` 决定（缺省 `false`＝单对象，形状与旧行为完全一致）；`true` 时模型返回数组、**最多 3 条**侧重点不重复的看点，出参逐条做同样的长度校验并丢弃空项（全空 → 502 可重试）。前端 SHALL 给一个「多给几个看点」入口，结果并列展示、**每条各自「用这条」采纳**（`value`→短标签、`note`→主输入框）。
