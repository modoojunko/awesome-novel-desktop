# entitlement-sync Specification

## Purpose

定义 C端×S端 套餐权益同步契约：S端 从权益记录×档位目录配置计算并下发用户权益快照（features/limits），C端 缓存快照、按路由刷新、以快照驱动功能判定，并在快照异常时三段式自愈（重同步→档位标准兜底+客服提示）。S端 是唯一事实源，C端 不自判档位。
